...
- **E2E Flow**: One `BpoFlowCard` per BPO showing 5-stage horizontal funnel (Created → Approved → Released → Shipped → Received). Each stage has icon circle, qty bar (height shrinks with qty drop-off), qty number and %. Waterfall summary line at bottom. Click any stage to drill-down inline RPO/ASN table filtered to that stage. Component lives at `src/components/orders/BpoFlowCard.tsx`.
