...
export function KpiCard({
  ...
  return (
    ...
    >
      {/* Tone strip — 3px chiều dọc bên trái */}
      <span aria-hidden className={cn("absolute left-0 top-0 bottom-0 w-[3px]", TONE_STRIP[tone])} />

      {/* Header: title + icon */}
      <div className="flex items-start justify-between gap-2 mb-1.5">
        <p className="text-table-sm font-medium text-text-2 leading-snug">{title}</p>
        {Icon && (
          <div className={cn("shrink-0 rounded-md p-1", TONE_CHIP[tone])}>
      ...
      <div className="flex items-baseline gap-1.5">
        ...
        {unit && <span className="text-table-sm text-text-3">{unit}</span>}
      </div>
      ...
      {(trend || hint) && (
        <div className="mt-2 flex items-center gap-1.5 flex-wrap">
          ...
          {hint && (
            <span className="text-caption text-text-3 inline-flex items-center gap-1">
